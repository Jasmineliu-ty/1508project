...
def plot_num_feater_change(model,model_pruned):

    filters_model1 = get_num_filters(model)
    filters_model2 = get_num_filters(model_pruned)

    layers = list(range(1, len(filters_model1) + 1))

    plt.figure(figsize=(10, 6))
    plt.plot(layers, filters_model1, label="Original Model", marker='o')
    plt.plot(layers, filters_model2, label="Pruned Model", marker='x')
    # plt.title("Comparison of Number of Filters in CNN Layers")
...
# input_tensor = transform(Image.open("test\dogs\dog.662.jpg").convert("RGB")).unsqueeze(0)
# feature_map = get_feature_map(model_pruned, layer_name="24", input_tensor=input_tensor)  # 修改目标层
# visualize_feature_maps_combined(feature_map, num_maps=16, layer_name="Conv Layer 24")
