...
class FilterPrunner:
    ...
    def lowest_ranking_filters(self, num):
        data = []
        # print(self.filter_ranks[0], 'type is:', type(self.filter_ranks[0]))
        for i in sorted(self.filter_ranks.keys()):
            for j in range(self.filter_ranks[i].shape[0]): # changed here!!! originally .size(0)
                data.append(
                    (self.activation_to_layer[i], j, self.filter_ranks[i][j].item()))
